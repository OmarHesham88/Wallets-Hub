...
import { CircleDollarSign, ReceiptText, Smartphone, WalletCards } from "lucide-react";
...
export default function DashboardPage() {
  ...
  const rows = dashboard.data?.latest ?? []; const today = dashboard.data?.today.find((item) => item.currencyCode === "EGP");
  ...
  return <Shell>
    ...
    <div className="stats"><div className="stat"><CircleDollarSign/><span>Confirmed today</span><strong>{money(today?.amount ?? 0)}</strong></div><div className="stat"><ReceiptText/><span>Confirmed payments today</span><strong>{today?.count ?? 0}</strong></div><div className="stat"><WalletCards/><span>Accessible wallets</span><strong>{(wallets.data ?? []).filter((item) => item.isActive).length}</strong></div></div>
