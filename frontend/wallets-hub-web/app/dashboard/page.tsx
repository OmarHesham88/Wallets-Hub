...
export default function DashboardPage() {
  ...
  const egp=confirmed.filter(x=>x.currencyCode==="EGP").reduce((s,x)=>s+x.amount,0); const usd=confirmed.filter(x=>x.currencyCode==="USD").reduce((s,x)=>s+x.amount,0); const usdt=confirmed.filter(x=>x.currencyCode==="USDT").reduce((s,x)=>s+x.amount,0);
  return <Shell><div className="page-head"><div><span className="eyebrow">Live operations</span><h1>Good day, {me.data?.displayName?.split(" ")[0] ?? "there"}</h1><p>Monitor incoming wallet payments and keep the review queue moving.</p></div><Link className="btn" href="/receipts"><CircleDollarSign size={18}/>Review receipts</Link></div>
    <div className="stats"><div className="stat"><Clock3/><span>Pending review</span><strong>{pending.length}</strong></div><div className="stat"><CheckCircle2/><span>Confirmed today · EGP</span><strong>{money(egp,"EGP")}</strong></div><div className="stat"><CircleDollarSign/><span>Confirmed today · USD</span><strong>{money(usd,"USD")}</strong></div><div className="stat"><CircleDollarSign/><span>Confirmed today · USDT</span><strong>{money(usdt,"USDT")}</strong></div><div className="stat"><WalletCards/><span>Accessible wallets</span><strong>{(wallets.data??[]).filter(x=>x.isActive).length}</strong></div></div>
