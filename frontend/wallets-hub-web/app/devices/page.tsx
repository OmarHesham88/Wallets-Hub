...
import { Activity, BatteryWarning, Copy, Edit3, Plus, RefreshCw, Smartphone, Trash2, Wifi, WifiOff, X } from "lucide-react";
import { Shell } from "@/components/shell";
import { api } from "@/lib/api";

type Device = { id: string; name: string; platform: string; isActive: boolean; pairedAtUtc?: string; lastSeenAtUtc?: string; lastHeartbeatAtUtc?: string; lastSmsAtUtc?: string; lastNotificationAtUtc?: string; lastCaptureAtUtc?: string; appVersion?: string; androidVersion?: string; pendingUploadCount: number; failedUploadCount: number; smsPermissionGranted: boolean; notificationPermissionGranted: boolean; batteryOptimizationIgnored: boolean; walletCount: number };
...
export default function DevicesPage() {
  ...
  return <Shell>
    <div className="page-head"><div><span className="eyebrow">Capture network</span><h1>Devices</h1><p>Heartbeat, permissions, app version, queue health, and capture activity for every paired phone.</p></div><div className="button-row"><Link className="btn btn-secondary" href="/capture-health"><Activity size={17}/>Capture inbox</Link><button className="btn" onClick={() => { setPairing(undefined); setCreateOpen(true); }}><Plus size={18}/>Pair device</button></div></div>
    ...
    <div className="grid">{(devices.data ?? []).map((device) => { const activityAt = device.lastHeartbeatAtUtc ?? device.lastSeenAtUtc; const online = device.isActive && activityAt && devices.dataUpdatedAt - new Date(activityAt).getTime() < 20 * 60 * 1000; return <article className="card" key={device.id}>
      <div className="card-top"><div className="card-icon"><Smartphone/></div><span className={`badge ${online ? "success" : "danger"}`}>{online ? <><Wifi size={12}/>Online</> : <><WifiOff size={12}/>{device.isActive ? "Offline" : "Disabled"}</>}</span></div>
      ...
      <div className="diagnostic-list"><Diagnostic label="Heartbeat" value={format(device.lastHeartbeatAtUtc)}/><Diagnostic label="Last receipt capture" value={format(device.lastCaptureAtUtc)}/><Diagnostic label="Last SMS" value={format(device.lastSmsAtUtc)} ok={device.smsPermissionGranted}/><Diagnostic label="Last Binance notification" value={format(device.lastNotificationAtUtc)} ok={device.notificationPermissionGranted}/><Diagnostic label="Upload queue" value={`${device.pendingUploadCount} pending · ${device.failedUploadCount} failed`} ok={device.failedUploadCount === 0}/><Diagnostic label="Battery protection" value={device.batteryOptimizationIgnored ? "Unrestricted" : "Optimization may stop capture"} ok={device.batteryOptimizationIgnored}/></div>
