...
export default function DevicesPage() {
  ...
  return <Shell>
    <div className="page-head"><div><span className="eyebrow">Capture network</span><h1>Devices</h1><p>See whether each wallet phone is connected and capturing payments.</p></div><button className="btn" onClick={() => { setPairing(undefined); setCreateOpen(true); }}><Plus size={18}/>Pair device</button></div>
    ...
    <div className="grid">{(devices.data ?? []).map((device) => { const health = deviceHealth(device, devices.dataUpdatedAt); return <article className="card" key={device.id}>
      ...
      <h2>{device.name}</h2><p>{device.walletCount} active wallet{device.walletCount === 1 ? "" : "s"}</p>
      <div className="device-client-summary"><div><span>Connection</span><strong>{health.label}</strong></div><div><span>Last activity</span><strong>{relativeTime(device.lastCaptureAtUtc ?? device.lastSeenAtUtc, devices.dataUpdatedAt)}</strong></div></div>
      <details className="technical-details"><summary>Technical details</summary><div className="diagnostic-list"><Diagnostic label="Last server contact" value={format(device.lastSeenAtUtc)}/><Diagnostic label="Heartbeat" value={format(device.lastHeartbeatAtUtc)} ok={Boolean(device.lastHeartbeatAtUtc)}/><Diagnostic label="Last receipt capture" value={format(device.lastCaptureAtUtc)}/><Diagnostic label="SMS capture permission" value={format(device.lastSmsAtUtc)} ok={device.smsPermissionGranted}/><Diagnostic label="App notification capture" value={device.lastAxisNotificationAtUtc ? `Last checked ${format(device.lastAxisNotificationAtUtc)}` : device.axisNotificationAccessGranted ? "Enabled · no payment notification yet" : "Permission required"} ok={device.axisNotificationAccessGranted}/><Diagnostic label="App version" value={`${device.appVersion ?? "Unknown"}${device.androidVersion ? ` · Android ${device.androidVersion}` : ""}`}/><Diagnostic label="Upload queue" value={`${device.pendingUploadCount} pending · ${device.failedUploadCount} failed`} ok={device.failedUploadCount === 0}/><Diagnostic label="Battery protection" value={device.batteryOptimizationIgnored ? "Unrestricted" : "Optimization may stop capture"} ok={device.batteryOptimizationIgnored}/></div></details>
