...
export default function ReportsPage() {
  const [from, setFrom] = useState(""); const [to, setTo] = useState(""); const [walletId, setWalletId] = useState(""); const [deviceId, setDeviceId] = useState(""); const [provider, setProvider] = useState(""); const [currency, setCurrency] = useState("EGP");
  const params = useMemo(() => queryString({ from: from ? new Date(`${from}T00:00:00`).toISOString() : "", to: to ? new Date(`${to}T23:59:59.999`).toISOString() : "", walletId, deviceId, provider, currency: currency || "" }), [currency, deviceId, from, provider, to, walletId]);
  ...
  const data = report.data; const total = data?.totals.find((item) => item.currencyCode === currency); const previous = data?.previousTotals.find((item) => item.currencyCode === currency); const change = previous?.amount ? ((total?.amount ?? 0) - previous.amount) / previous.amount * 100 : null;
  ...
  function resetFilters() { setFrom(""); setTo(""); setWalletId(""); setDeviceId(""); setProvider(""); setCurrency("EGP"); }
  const hasExtraFilters = Boolean(walletId || deviceId || provider || currency !== "EGP" || !isPreset(30));
  ...
  return <Shell>
    ...
    <section className="panel filter-panel filter-panel-modern report-filter-panel">
      ...
      <div className="report-filter-toolbar">
        ...
        <label className="select-control"><span>Provider</span><select value={provider} onChange={(event) => setProvider(event.target.value)}><option value="">All providers</option><option>Vodafone Cash</option><option>InstaPay</option><option>Binance</option></select></label>
        <label className="select-control currency-control"><span>Currency</span><select value={currency} onChange={(event) => setCurrency(event.target.value)}>{currencies.map((item) => <option key={item}>{item}</option>)}</select></label>
      </div>
    </section>
    ...
    <section className="panel quality-panel"><div><span className="eyebrow">Data quality</span><h2>Capture reliability</h2></div><div><AlertTriangle/><strong>{data?.quality.unmatchedCaptures ?? 0}</strong><span>Unmatched captures</span></div><div><AlertTriangle/><strong>{data?.quality.duplicateCaptures ?? 0}</strong><span>Safe duplicates</span></div><div><AlertTriangle/><strong>{data?.quality.rejectedCaptures ?? 0}</strong><span>Rejected captures</span></div><div><AlertTriangle/><strong>{data?.quality.missingSender ?? 0}</strong><span>Missing sender</span></div><div><AlertTriangle/><strong>{data?.quality.failedUploads ?? 0}</strong><span>Failed uploads</span></div><div><TrendingUp/><strong>{data?.quality.newSenders ?? 0} / {data?.quality.returningSenders ?? 0}</strong><span>New / returning senders</span></div></section>
  </Shell>;
}
